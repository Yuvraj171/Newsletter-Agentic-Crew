...
"""
# -------------------------------------------------------------------
# Existing topic_definitions and schemas remain UNCHANGED
# -------------------------------------------------------------------
# (All the topic_definitions list and schemas you already have above)
# -------------------------------------------------------------------

# Minimal default topic_definitions to avoid "not defined" errors.
# Replace these entries with your actual topics/schemas as needed.
topic_definitions = [
    {
        "topic": "AI at Work",
        ...
        - You MUST follow these five exact headings in this order: (**Note**: follow the provided headings only)
            1) Main Definition and Key Features: **single selected tool**
            2) What's New
            3) Why This Matters
        ...
        -   **Headings**: You MUST keep these five exact headings:
            1) Main Definition and Key Features: [Selected Tool Name]
            2) What's New
            3) Why This Matters
            4) How BEST Group Could Use It
            5) How to Use It
    ...
    },
    {
        ...
        -   **Headings**: You MUST keep these four exact headings:
            1) Top Time-Saving Windows Shortcuts
            2) Top Time-Saving Mac Shortcuts
            3) Quick Fixes for Everyday Tech Headaches
            4) Pro-Tips
...
def run():
    ...
    for definition in topic_definitions:
        ...
        generated_files[topic] = f"{topic_slug}_{month_name}.html"

    print("\n--- ✅ ALL CONTENT PAGES GENERATED ---")
    print(generated_files)

    # =========================
    # 2️⃣ CONSOLIDATE INTO ONE EMAIL-SAFE HTML
    # =========================
    print("\n--- 🧩 Consolidating all topic HTML into one email file ---")
    today = datetime.date.today()
    month_name = today.strftime("%B")
    ...
    for definition in topic_definitions:
        topic = definition["topic"]
        topic_slug = definition["topic_slug"]
        output_file = f"outputs/{topic_slug}_{month_name}.html"

        try:
            with open(output_file, "r", encoding="utf-8") as f:
                html = f.read()
            # Extract body content (ignore outer tags)
            m = re.search(r"<body[^>]*>([\s\S]*?)</body>", html, flags=re.IGNORECASE)
            body_inner = m.group(1).strip() if m else html
            section_html = (
                f'<hr style="border:none;border-top:1px solid #ddd;margin:24px 0;">'
                f'<h2 style="font-family:Arial,Helvetica,sans-serif;margin:0 0 8px;">{topic}</h2>'
                f"{body_inner}"
            )
            consolidated_sections.append(section_html)
        except FileNotFoundError:
            print(f"⚠️ Skipping missing file: {output_file}")
...
<title>BEST Group Tech Newsletter — {month_name}</title>
<style>
  body {{ margin:0; padding:16px; background:#ffffff; color:#222; }}
  h1 {{ font-family:Arial,Helvetica,sans-serif; font-size:22px; margin:0 0 12px; }}
  p, li {{ font-family:Arial,Helvetica,sans-serif; font-size:14px; line-height:1.5; }}
  a {{ color:#0b62d6; }}
  table {{ border-collapse: collapse; width:100%; }}
  th, td {{ border:1px solid #ddd; padding:6px; font-family:Arial,Helvetica,sans-serif; font-size:14px; }}
</style>
</head>
<body>
  <h1>BEST Group Tech Newsletter — {month_name}</h1>
  <p style="font-family:Arial,Helvetica,sans-serif; font-size:14px;">
    Here are this month’s highlights across all topics.
  </p>
  {''.join(consolidated_sections)}
</body>
</html>"""

    out_email_path = f"outputs/newsletter_email_{month_name}.html"
    with open(out_email_path, "w", encoding="utf-8") as f:
        f.write(email_html)
    print(f"✅ Consolidated newsletter saved to: {out_email_path}")

    # =========================
    # 3️⃣ EMAIL DISPATCH USING GMAIL AGENT
    # =========================
    print("\n--- 📧 Sending newsletter email ---")
    with open(out_email_path, "r", encoding="utf-8") as f:
        consolidated_body = f.read()

    recipients = [
        "Gavvala.kiran@bestgroup.co.in",
        "tanya.gosain@bestgroup.co.in",
    ]  # 🔁 Replace with actual distribution list

    subject = f"BEST Group Tech Newsletter — {month_name}"

    email_agent = create_email_agent()
    email_task = build_email_task(email_agent, recipients, subject, consolidated_body)
